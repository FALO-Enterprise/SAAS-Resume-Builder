...
import { useLocale, useTranslations } from 'next-intl';import { useRouter } from 'next/navigation';
...
function AuthInput({ icon: Icon, type, placeholder, value, onChange, error, rightSlot, label }: {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div
        className={`group flex items-center rounded-xl bg-card px-4 transition-all focus-within:bg-card-hover ${
          error
 ...
 const t = useTranslations('auth.signup');
  ...
  return (
    <div className="flex flex-col gap-5">
      <AuthInput icon={User} type="text" placeholder={t('nameExample')} label={t('fullNamePlaceholder')}
      ...
      <div className="flex flex-col gap-2">
        <AuthInput icon={Lock} type={showPw ? 'text' : 'password'} placeholder={t('passwordInputPlaceholder')} label={t('passwordPlaceholder')}
      ...
      <AuthInput icon={Lock} type={showConfirm ? 'text' : 'password'}placeholder={t('confirmInputPlaceholder')} label={t('confirmPasswordPlaceholder')}
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (data: RegisterData) => {
    ...
    try {
      ...
      if (resData.token) localStorage.setItem('resumax_token', resData.token);
      setName(data.name);
      setSuccess(true);
