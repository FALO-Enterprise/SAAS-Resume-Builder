              ...
              <div className="mt-auto pt-7 sm:pt-9">
                ...
                {selectedFile ? (
                  <div className="rounded-2xl border border-green/30 bg-green/10 p-4">
                    ...
                    {/*
                     * There's no resume-parsing endpoint wired up yet, so a
                     * selected file previously had no way to actually go
                     * anywhere — a dead end. Be upfront about it and route
                     * into the guided builder instead of leaving the click
                     * do nothing.
                     */}
                    <p className="mt-3 text-xs font-semibold leading-5 text-primary! opacity-75">
                      {t("upload.notAvailable")}
                    </p>

                    <Link
                      href={`/${locale}/templates?source=scratch`}
                      className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-green/40 bg-green/10 px-4 text-sm font-black text-green! no-underline transition-colors hover:border-green hover:bg-green/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green/40"
                    >
                      {t("upload.continueManually")}
