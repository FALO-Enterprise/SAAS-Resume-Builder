...
function PasswordStrength({ password }: { password: string }) {
  ...
  const barColor = ["#f87171", "#f87171", "#fb923c", "#facc15", "#4ade80"][
    score
  ];
  const label = [
    "",
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        ...
        <span
          className="min-w-11 text-right text-[11px] font-semibold"
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1">
        {checks.map((c) => (
          <div key={c.label} className="flex items-center gap-1.25">
            ...
            <span
              className={`text-[11px] ${c.pass ? "text-secondary" : "text-muted"}`}
            >
              {c.label}
            </span>
          </div>
...
function RegisterForm({
  ...
  const validate = (): boolean => {
    ...
    if (!form.confirm) e.confirm = t("errors.confirmRequired");
      ...
      e.confirm = t("errors.confirmMismatch");
...
export default function RegisterPage() {
  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-base px-6 py-10">
      {/* Background glows */}
      <div className="fixed top-1/6 left-1/6 w-125 h-125 glow-gold rounded-full blur-[80px] pointer-events-none" />
      <div className="fixed bottom-1/6 right-1/12 w-100 h-100 glow-azure rounded-full blur-3xl pointer-events-none" />
      <div className="fixed top-1/2 right-1/12 w-75 h-75 glow-gold rounded-full blur-3xl pointer-events-none" />
