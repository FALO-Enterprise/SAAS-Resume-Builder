          ...
          {keys.map((key, i) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative glass rounded-2xl p-7 border border-edge ${color.border} transition-all duration-300 overflow-hidden hover:-translate-y-1`}
              >
                {/* Background gradient on hover */}
                <div
                  className={`absolute inset-0 bg-linear-to-br ${color.bg} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
                />

                <div className="relative z-10">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 border ${color.iconWrap}`}
                  >
                ...
                {/* Accent bottom line */}
                <div
                  className={`absolute bottom-0 left-0 right-0 h-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-linear-to-r from-transparent ${color.line} to-transparent`}
                />
              </motion.div>
            );
          })}
