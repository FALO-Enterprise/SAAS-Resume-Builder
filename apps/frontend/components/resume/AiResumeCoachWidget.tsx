...
export default function AiResumeCoachWidget({
  ...
  const fetchAnalysis = async () => {
    ...
    try {
      ...
      toast.error('AI Coach analysis failed. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleApplyTipFix = (tip: AiCoachTip) => {
    ...
    toast.success(`Applied AI Coach fix: ${tip.title}`);
  ...
  return (
    <>
      ...
      <div className={`fixed ${bottomOffsetClassName} right-6 z-40`}>
        <motion.button
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`fixed ${bottomOffsetClassName === 'bottom-6' ? 'bottom-20' : 'bottom-20'} right-6 z-50 w-[420px] max-w-[calc(100vw-2rem)] h-[620px] max-h-[82vh] bg-slate-900/95 backdrop-blur-xl border border-amber-500/30 rounded-2xl shadow-2xl flex flex-col overflow-hidden text-slate-100`}
          >
            {/* Header */}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    <h3 className="font-bold text-sm text-slate-100">Gemini AI Coach</h3>
                  ...
                  <p className="text-[11px] text-slate-400">Real-time ATS Scoring & Executive Rewriting</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button
                  onClick={() => {
                    ...
                    toast.info('Rescanning all resume sections for new recommendations...');
                    void fetchAnalysis();
                  }}
                  disabled={isLoading}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs">
              {isLoading && !analysis ? (
                <div className="h-full flex flex-col items-center justify-center gap-3 text-slate-400 py-12">
                  ...
                  <p className="text-xs font-medium">Analyzing resume against global ATS benchmarks...</p>
                </div>
              ) : analysis ? (
                <>
                  {/* Polished Score Card with SVG Radial Ring Gauge */}
                  <div className="relative bg-gradient-to-br from-slate-800 via-slate-900 to-slate-950 p-4 rounded-xl border border-amber-500/30 shadow-lg flex items-center justify-between overflow-hidden">
                  ...
                  <div className="space-y-2.5">
                    <h4 className="font-bold text-slate-200 text-xs flex items-center justify-between">
                      <span className="flex items-center gap-1.5">
                        ...
                        Executive Coaching Fixes ({analysis.tips.filter((t) => !isTipApplied(t)).length})
                      </span>
                      {isLoading && <span className="text-[10px] text-amber-400 animate-pulse">Refreshing score...</span>}
                    </h4>
                    <AnimatePresence>
                      {analysis.tips
                        ...
                        .map((tip) => (
                          ...
                          >
                            ...
                            {tip.suggestedFix && onApplyFix && (
                              ...
                              >
                                <Check size={13} />
                                {tip.suggestedActionText || 'Apply Fix to Resume'}
                              </button>
                            )}
                          </motion.div>
                        ))}
      ...
      <AnimatePresence>
        {showUpgradeModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
            ...
            >
              <div className="w-14 h-14 mx-auto rounded-2xl bg-gradient-to-tr from-amber-500 to-yellow-400 flex items-center justify-center text-slate-950 shadow-lg shadow-amber-500/30">
                <Crown size={28} />
              </div>
              <h3 className="text-lg font-black text-slate-100">Enterprise AI Resume Coach</h3>
              <p className="text-xs text-slate-300 leading-relaxed">
                The AI Resume Coach provides real-time ATS scoring, keyword optimization, and 1-click executive rewriting using Gemini AI. Upgrade your plan to Enterprise to unlock full coach capabilities.
              </p>
              <div className="flex gap-3 pt-2">
                ...
                <Link
                  href="/pricing"
                  className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-500 text-slate-950 text-xs font-extrabold flex items-center justify-center gap-1.5 shadow-md shadow-amber-500/20 hover:shadow-amber-500/40 transition-all cursor-pointer"
                >
